...
app.post("/validate-message", (req, res) => {
  ...
  const words = message.trim().split(/\s+/).map(word => word.toLowerCase()); // Normalize words
...
const generateWordMap = (messageWords, keyWords) => {
  let availableWords = [...commonWords]; // Use the common words list
  let mapping = {};
  ...
  messageWords.forEach((word, index) => {
    if (!mapping[word]) {
      ...
      let newWord = availableWords.splice(shift, 1)[0]; // Pick a word deterministically
      mapping[word] = newWord || word; // Ensure mapping exists
    }
  });
...
const encryptMessage = (message, keyWords) => {
  ...
  wordMap = generateWordMap(words, keyWords); // Generate mapping
  return words.map(word => wordMap[word] || word).join(" ");
};

// Decrypt a message (reverse the word substitution)
const decryptMessage = (message) => {
  let reversedMap = Object.fromEntries(Object.entries(wordMap).map(([k, v]) => [v, k]));
  return message.split(" ").map(word => reversedMap[word] || "???").join(" ");
};
...
app.post("/set-message", (req, res) => {
  ...
  if (!phrase || !message) {
    return res.status(400).json({ error: "Secret phrase and message are required." });
  }
  ...
  if (invalidWords.length > 0) {
    return res.status(400).json({ error: "Invalid words in message.", invalidWords });
  }
...
app.post("/decrypt", (req, res) => {
  ...
  const isCorrect = JSON.stringify(inputPhrase) === JSON.stringify(secretPhrase);

  if (isCorrect) {
    // Correct phrase, decrypt properly
    const decryptedMessage = decryptMessage(encryptedMessage);
    res.json({ decryptedMessage });
  } else {
    // Incorrect phrase, generate structured gibberish
    let scrambledWords = encryptedMessage.split(" ");
    let gibberish = scrambledWords.map(() => commonWords[Math.floor(Math.random() * commonWords.length)]).join(" ");
